...
from tgbot.keyboards.inline import (
    notification_menu,
    inside_notification_menu,
    back_to_notification_menu,
    select_date_menu,
    agree_menu,
    back_to_notification_edit_menu,
    confirm_editing_notification,
)
from tgbot.messages.reply_buttons import notifications_button
from tgbot.misc.states import (
    AddNotificationState,
    EditNotificationDescTitle,
...
async def all_notifications(
    update: Message | CallbackQuery, state: FSMContext, user: dict
):
    await state.clear()
    if isinstance(update, Message):
        await update.answer(
            notifications_menu_message.get(user.lang),
            reply_markup=await notification_menu(user.id, user.lang),
        )
        return
...
async def select_notification(call: CallbackQuery, user: dict):
    ...
    notification = await AsyncORM.notifications.get(notif_id)
    await call.message.edit_text(
        notification_desc(notification, user.lang),
        reply_markup=inside_notification_menu(
            lang=user.lang,
            notification_id=notif_id,
            enabled=notification.is_on,
        ),
    )
...
async def enable_notification(
    call: CallbackQuery, user: dict, scheduler: AsyncIOScheduler
):
    ...
    if notification.is_on == 1:
        await AsyncORM.notifications.update(notification_id, is_on=0)
        if job:
            job.remove()
        enabled = 0
    else:
        await AsyncORM.notifications.update(notification_id, is_on=1)
        if not job:
    ...
    await call.message.edit_text(
        notification_desc(notification, user.lang),
        reply_markup=inside_notification_menu(
            lang=user.lang, notification_id=notification_id, enabled=enabled
        ),
    )
...
async def edit_notification(call: CallbackQuery, state: FSMContext, user: dict):
    ...
    if option == "delete":
        await AsyncORM.notifications.delete(int(notif_id))
        await call.message.edit_text(
            notification_deleted_message.get(user.lang),
            reply_markup=back_to_notification_menu(user.lang),
        )
        return
    ...
    if option in ("title", "description"):
        ...
        msg = await call.message.edit_text(
            edit_tile_desc_msg(option, notification.__dict__.get(option), user.lang),
            reply_markup=back_to_notification_edit_menu(user.lang, notif_id),
        )
        ...
        await call.answer(
            "❗️This function in development right now."
            if user.lang == "en"
            else "❗️Эта функция находится в разработке"
        )
...
async def receive_title_desc(message: Message, state: FSMContext, user: dict):
    ...
    msg_id, notif_id, option, value = (
        data.get("msg_id"),
        data.get("notif_id"),
        data.get("option"),
        message.text,
    )
    notif = await AsyncORM.notifications.get(notif_id)
    await message.delete()
    await message.bot.edit_message_text(
        edit_tile_desc_confirm(option, notif.__dict__.get(option), value, user.lang),
        chat_id=message.from_user.id,
        message_id=msg_id,
        reply_markup=confirm_editing_notification(user.lang, notif_id),
    )
...
@notifications_router.callback_query(
    F.data == "confirm_editing_notif", EditNotificationDescTitle.confirm
)
async def confirm_edit(call: CallbackQuery, state: FSMContext, user: dict):
    data = await state.get_data()
    notif_id, option, new_value = (
        data.get("notif_id"),
        data.get("option"),
        data.get("new_value"),
    )
    notif = await AsyncORM.notifications.get(notif_id)
    await AsyncORM.notifications.update(notif_id, **{option: new_value})
    await call.message.edit_text(
        successfully_edited_msg(
            option, notif.__dict__.get(option), new_value, user.lang
        ),
        reply_markup=back_to_notification_edit_menu(user.lang, notif_id),
    )
    await state.clear()
...
async def add_notification_ask_for_title(
    call: CallbackQuery, state: FSMContext, user: dict
):
    msg = await call.message.edit_text(
        send_title.get(user.lang),
        reply_markup=back_to_notification_menu(user.lang),
    )
...
async def add_notification_ask_for_desc(
...
async def add_notification_ask_for_date(
    message: Message, state: FSMContext, user: dict
):
    data = await state.get_data()
    ...
    now = datetime.now()
    await message.bot.edit_message_text(
        choose_date(title, desc, user.lang),
        chat_id=message.from_user.id,
        message_id=msg_id,
...
@notifications_router.callback_query(
    F.data.startswith("add_choose_date:"), AddNotificationState.AN3
)
async def add_notification_receive_date(
    call: CallbackQuery, state: FSMContext, user: dict
):
    data = await state.get_data()
    ...
    if done == "done":
        ...
        await state.set_state(AddNotificationState.AN3)
        await call.message.edit_text(
            choose_time(title, desc, date.strftime("%Y.%m.%d"), user.lang),
            reply_markup=back_to_notification_menu(user.lang),
        )
    ...
    date = datetime(year=int(year), month=int(month), day=int(day))
    if date < datetime.now() and date.strftime("%Y.%m.%d") != datetime.now().strftime(
        "%Y.%m.%d"
    ):
        await call.answer(past_date.get(user.lang), show_alert=True)
        return
    await call.message.edit_text(
        choose_date(title, desc, user.lang),
...
async def add_notification_receive_time(
    message: Message, state: FSMContext, user: dict
):
    data = await state.get_data()
    ...
    if len(time) == 2 and time[0].isdigit() and time[1].isdigit():
        ...
        await message.bot.edit_message_text(
            last_agree(
                title, desc, date.strftime("%Y.%m.%d"), ":".join(time), user.lang
            ),
            chat_id=message.from_user.id,
            message_id=msg_id,
            reply_markup=agree_menu(user.lang),
        )
        await state.update_data(time=":".join(time))
    else:
        await message.bot.edit_message_text(
            wrong_format.get(user.lang), chat_id=message.from_user.id
        )


@notifications_router.callback_query(
    F.data.startswith("done_adding"), AddNotificationState.AN4
)
async def add_notification_done(
    call: CallbackQuery, state: FSMContext, user: dict, scheduler: AsyncIOScheduler
):
    data = await state.get_data()
    ...
    notification = await AsyncORM.notifications.create(
        user_id=call.from_user.id,
        description=desc,
        date_completed=f"{date} {time}",
        title=title,
    )
    await add_notification_job(notification, call.bot, scheduler)
    await call.message.edit_text(
        notification_done.get(user.lang),
        reply_markup=back_to_notification_menu(user.lang),
    )
    await state.clear()
